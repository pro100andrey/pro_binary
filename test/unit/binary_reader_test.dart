  ...
  group('BinaryReader', () {
    ...
    group('offset getter', () {
      ...
      group('Malformed UTF-8', () {
        ...
        test(
          'readString with allowMalformed=false throws on invalid UTF-8',
          () {
      ...
      group('rebind', () {
        test('rebind replaces buffer and resets offset to 0', () {
          ...
          expect(reader.readUint8(), equals(0x20));
        });

        test('rebind after partial reads', () {
          final buffer1 = Uint8List.fromList([0x01, 0x02, 0x03, 0x04, 0x05]);
          final reader = BinaryReader(buffer1)..readUint32();
          expect(reader.offset, equals(4));

          final buffer2 = Uint8List.fromList([0xAA, 0xBB]);
          reader.rebind(buffer2);

          expect(reader.offset, equals(0));
          expect(reader.readUint8(), equals(0xAA));
          expect(reader.readUint8(), equals(0xBB));
        });
        ...
        test('rebind preserves reader identity', () {
          final buffer1 = Uint8List.fromList([0x01]);
          final reader = BinaryReader(buffer1);
          final readerRef = reader;

          final buffer2 = Uint8List.fromList([0x02, 0x03]);
          reader.rebind(buffer2);

          expect(readerRef.length, equals(2));
          expect(readerRef.readUint8(), equals(0x02));
        });

        test('rebind multiple times', () {
          final reader = BinaryReader(Uint8List.fromList([0x01]));

          for (var i = 0; i < 5; i++) {
            final buffer = Uint8List.fromList([i, i + 1, i + 2]);
            reader.rebind(buffer);
            expect(reader.offset, equals(0));
            expect(reader.readUint8(), equals(i));
          }
        });

        test('rebind with data that has non-zero buffer offset', () {
          final original = Uint8List.fromList(
            List<int>.generate(10, (_) => 0),
          );
          final sliced = original.buffer.asUint8List(5, 5);
          sliced[0] = 0x42;
          sliced[1] = 0x43;

          final reader = BinaryReader(Uint8List.fromList([0x00]))
            ..rebind(sliced);
